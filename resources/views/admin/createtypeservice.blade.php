      ...
      <div class="page-content">
        <div class="page-header">
          <div class="container-fluid">
            ...
            <br>
            <form action="{{ url ('createTypeService') }}" method="post">
                @csrf
                <div>
                    <h4>اسم نوع الخدمة</h4>
                    <input type="text" name="type" >
                    <div class="form-group">
    <h4> اسم الصالة المرتبطة </h4>
    <select name="hall_id" class="form-control">
        <option value="" disabled selected>اختر  الصالة</option>
        @foreach ($hall as $type) <!-- استخدام المتغير الصحيح -->
            <option value="{{ $type->id }}">{{ $type->name }}</option> <!-- قيمة الـ ID واسم النوع -->
        @endforeach
    </select>
</div>
                    <br>
          ...
          <div>
            ...
            <table class="table_deg">
                <tr>
                    <th>اسم الصالة المرتبطة </th>
                    <th>حذف</th>
                    <th>تعديل</th>
                </tr>
                @foreach ($typeservice as $data)
                <tr>
                    <td>{{ $data->type }}</td>
                    <td>
